...
def translate_with_local_qwen(text: str, context_prev: str = "") -> Tuple[str, Dict[str, any]]:
    ...
    sys_pass1 = [
        ...
        "متن انگلیسی زیر را با حفظ دقیق بار معنایی، تمام کلمات و لحن به فارسی شیوا ترجمه کنید.",
        "قوانین اجباری:",
        "۱. عدم سانسور یا تلطیف: تمام فحش‌ها، ناسزاها و اصطلاحات کوچه و بازاری را دقیق، صریح و متناسب با لحن کاراکتر به معادل‌های طبیعی فارسی ترجمه کنید و به هیچ عنوان چیزی را سانسور یا حذف نکنید.",
        "۲. خط تیره (— یا –) اکیداً ممنوع است. از ویرگول یا ساختار طبیعی جمله استفاده کنید.",
        "۳. نیم‌فاصله‌ها را کاملاً دقیق رعایت کنید.",
        "۴. نقل‌قول‌ها داخل گیومه فارسی («...») باشند.",
        "۵. فقط ترجمه فارسی را خروجی دهید."
    ]
    if glossary_context:
    ...
    sys_pass2 = (
        ...
        "- هیچ سانسور یا تلطیف اعمال نکنید؛ تمام کلمات و اصطلاحات باید دقیق و کامل بمانند.\n"
