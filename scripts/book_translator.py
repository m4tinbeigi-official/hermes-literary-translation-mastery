...
۱. پرهیز مطلق از سانسور، تلطیف، یا حذف کلمات. تمام فحش‌ها، ناسزاها، کلمات رکیک و اصطلاحات کوچه و بازاری باید دقیق، صریح و متناسب با لحن و شخصیت به معادل‌های طبیعی و کوبنده فارسی ترجمه شوند.
۲. پرهیز مطلق از خط تیره (— یا –). از ویرگول، پرانتز یا ساختار اصیل فارسی استفاده کنید.
۳. رعایت کامل نیم‌فاصله (می‌شود، کتاب‌ها، خانه‌اش).
۴. قرار دادن دیالوگ‌ها داخل گیومه فارسی «...».
۵. گوینده و فعل دیالوگ باید پیش از گیومه بیاید (مانند: هری گفت: «...»).
۶. پایبندی کامل به واژه‌نامه و شناسنامه لحن شخصیت‌های مجموعه.
"""
...
def translate_book_series_aware(book_dir: str):
    ...
    for chunk_path in chunks:
        ...
        p2_sys = "شما ویراستار ادبی هستید. جملات را به فارسی اصیل، کوتاه و آهنگین بازنویسی کنید. خط تیره ممنوع. گوینده قبل از گیومه بیاید. هیچ سانسور یا تلطیف اعمال نکنید؛ تمام کلمات و اصطلاحات باید دقیق و کامل بمانند. فقط متن نهایی:"
